...
function updateResult(dices, sid) {
    ...
    if (sid !== latestResult["Phiên trước"]) {
        ...
        console.log(
            `[🎲 XD88 MD5 ✅] Phiên ${latestResult["Phiên trước"]} - ${d1}-${d2}-${d3} ➜ Tổng: ${total} | ${timeStr}`
        );
    }
}

const MD5_URL = "https://taixiumd5.system32-cloudfare-356783752985678522.monster/api/md5luckydice/GetSoiCau?access_token=";

async function fetchGameData() {
    try {
        const response = await axios.get(MD5_URL);
        ...
        console.error("❌ Lỗi khi lấy dữ liệu XD88 MD5:", error.message);
    }
}
...
module.exports = {
    ...
    startConnection: () => {
        console.log(`[🚀] Khởi tạo tự động lấy dữ liệu XD88 MD5...`);
